...
class IndexView(ListView):
    ...
    def get_queryset(self):
        ...
        return Reservation.objects.filter(
            timestamp__date__gte=self.date, timestamp__date__lt=max_date,
        ).order_by("timestamp")

    def get_context_data(self, **kwargs):
        resolution = timedelta(minutes=30)
        start = datetime(
            self.date.year, self.date.month, self.date.day, 10, 00, tzinfo=timezone.utc
        )
        ...
        kwargs["today"] = today
        kwargs["week"] = [today + timedelta(days=i) for i in range(0, 7)]
        kwargs["week_next1"] = [today + timedelta(days=i) for i in range(7, 14)]
        kwargs["week_next2"] = [today + timedelta(days=i) for i in range(14, 21)]
        kwargs["week_next3"] = [today + timedelta(days=i) for i in range(21, 28)]

        for reservation in self.object_list:
            for i in range(int(reservation.duration / resolution)):
...
class ConfirmReservation(DetailView):
    model = Reservation
    slug_url_kwarg = "code"
    slug_field = "code"
...
class CalendarView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        qs = Reservation.objects.order_by("timestamp").filter(timestamp__gte=today)
